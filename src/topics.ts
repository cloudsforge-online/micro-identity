...
export const AWAITING_REGISTRATION: Readonly<Record<string, ProposedTopic>> = Object.freeze({
  ...
  // `identity.password.reset_requested` was the fourth to leave this table, and it left the way
  // the doc above promises: contracts adopted the spec that used to sit here — character for
  // character, keyedBy and description included — `adoptedProposals()` turned non-empty, and this
  // suite failed until the entry was deleted from here.
  //
  // What is worth recording is how long it sat. This one was never DEPLOYED ahead of its contract
  // the way the verification topic was, so nothing broke and nobody looked: notify held a rule for
  // it and identity emitted it, and the two agreed for the whole life of the feature because they
  // had independently guessed the same string. The cost was invisible rather than absent —
  // `activity/src/ingest.ts` files an unregistered topic as `unclassified` / `internal` with
  // 90-day quarantine retention instead of the 730 days a security record gets, so the estate was
  // deleting its account-recovery trail early. It took an estate-wide check (`estate-topics`) to
  // see it, because no single repository could: from in here the topic looks fine. micro-org#263.
  'identity.role.changed': {
    reason:
