 ...
 *
 *   const session = await withOutbox(sql, SERVICE, async (tx, emit) => {
 *     const row = await insertSession(tx, input)
 *     emit({ topic: 'identity.session.created', key: row.id, payload: { id: row.id } })
 *     return row
 *   })
 *
