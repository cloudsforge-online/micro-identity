...
| `POST` | `/auth/handoff` | user | mints a 60-second, single-use, origin-bound SSO code (`src/server.ts:1035`) |
| `POST` | `/auth/handoff/redeem` | **no auth** (holds a code) | redeems it for tokens **in a response body, never a URL** (`src/server.ts:1043`) |
...
| `IDENTITY_KEY_SECRET` | — | **required, ≥32 chars, placeholders refused.** Wrong → no key can be unwrapped and nothing can be signed. Disclosed → the estate is forgeable (`src/env.ts:236`) |
...
| `IDENTITY_HANDOFF_ORIGINS` | `` (none) | comma-separated origins allowed to redeem a hand-off code. **Empty means none**, which is the safe default rather than the convenient one — an empty list makes SSO fail closed. Normalised through `origin()` so a trailing slash on one side is not a mismatch that reads to the user as "sign in bounced me" (`src/env.ts:219-223`, `:85-91`) |
